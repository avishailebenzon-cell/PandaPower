...
export async function fetchOutreachContacts(
  ...
  const response = await fetch(
    `${API_BASE}/admin/pandi/outreach/contacts?${params}`,
    { credentials: 'include' }
  );
...
export async function createCampaign(data: {
  ...
  const response = await fetch(
    `${API_BASE}/admin/pandi/outreach/campaigns`,
    {
      method: 'POST',
...
export async function previewCampaign(
  ...
  const response = await fetch(
    `${API_BASE}/admin/pandi/outreach/campaigns/${campaignId}/preview?${params}`,
    { credentials: 'include' }
  );
...
export async function sendCampaign(campaignId: string): Promise<SendResponse> {
  const response = await fetch(
    `${API_BASE}/admin/pandi/outreach/campaigns/${campaignId}/send`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function fetchCampaign(campaignId: string): Promise<Campaign> {
  const response = await fetch(
    `${API_BASE}/admin/pandi/outreach/campaigns/${campaignId}`,
    { credentials: 'include' }
  );
...
export async function fetchCampaigns(limit = 20, offset = 0): Promise<Campaign[]> {
  ...
  const response = await fetch(
    `${API_BASE}/admin/pandi/outreach/campaigns?${params}`,
    { credentials: 'include' }
  );
