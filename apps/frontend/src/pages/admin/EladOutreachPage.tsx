/**
 * Session 35: Elad Premium Client Outreach Campaign Management
...
import * as eladApi from '../../api/elad';
...
אני אלעד, בוט חברה ייצוא מיוחד של {company} 🤝

ניתנו לך הזדמנות להשתתף בתוכנית בחינם של שירותי הצבה.

אם אתה מעוניין, שמור את הודעה זו כדי שאוכל להזהות אותך בהמשך!

לשאלות: {phone}

(זה מוקד בדיקה מוגבל - תודה על הסבלנות)`;

export default function EladOutreachPage() {
  // State
  ...
  const [contacts, setContacts] = useState<eladApi.Contact[]>([]);
  const [selectedContacts, setSelectedContacts] = useState<eladApi.Contact[]>([]);
  const [messageTemplate, setMessageTemplate] = useState(DEFAULT_MESSAGE_TEMPLATE);
  const [campaignName, setCampaignName] = useState('');
  const [campaign, setCampaign] = useState<eladApi.Campaign | null>(null);
  const [preview, setPreview] = useState<eladApi.CampaignPreview | null>(null);
  ...
  const handleLoadContacts = async () => {
    ...
    setError(null);
    try {
      const data = await eladApi.fetchOutreachContacts(
        {
  ...
  const handlePreview = async () => {
    ...
    try {
      // Create campaign
      const newCampaign = await eladApi.createCampaign({
      ...
      // Get preview
      const previewData = await eladApi.previewCampaign(newCampaign.id, 50);
  ...
  const handleSendCampaign = async () => {
    ...
    setError(null);
    try {
      const result = await eladApi.sendCampaign(campaign.id);
      // Refresh campaign status
      const updatedCampaign = await eladApi.fetchCampaign(campaign.id);
            ...
            {contacts.length > 0 && (
              <Box sx={{ mt: 3 }}>
                <TextField
                  ...
                  placeholder="e.g., 'Premium Client Launch - May 2026'"
                  sx={{ mb: 2 }}
                />
