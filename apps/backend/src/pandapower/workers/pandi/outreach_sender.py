"""Celery task: Send Pandi outreach messages with rate limiting (Session 35)"""
...
async def _process_pandi_outreach_async(campaign_id: str) -> dict:
    ...
    try:  # pragma: no cover - unreachable, kept for easy re-enable
        ...
        # Get Pandi Green API client
        try:
            pandi_client = await get_green_api_client("pandi")
            if not pandi_client:
                raise Exception("Pandi WhatsApp not configured")
        except Exception as e:
            logger.error(f"Failed to get Green API client: {e}")
            return {"status": "failed", "reason": str(e)}

        # Get pending messages
        messages_result = await supabase.table("pandi_outreach_messages").select("*").eq(
        ...
        for i, msg in enumerate(messages):
            try:
                ...
                if not chat_id:
                    ...
                    await supabase.table("pandi_outreach_messages").update({
                        "status": "failed",
                ...
                result = await pandi_client.send_message(chat_id, message_text)
...
def process_pandi_outreach(self, campaign_id: str) -> dict:
    """Celery task to process Pandi outreach campaign
    ...
    """
    logger.info(f"Starting Pandi outreach task for campaign {campaign_id}")
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            result = loop.run_until_complete(_process_pandi_outreach_async(campaign_id))
