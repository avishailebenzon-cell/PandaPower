...
SCHEMA_MIGRATIONS = {
    ...
    """,
    "dana_conversations": """
    CREATE TABLE IF NOT EXISTS dana_conversations (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        title TEXT,
        status TEXT DEFAULT 'open',
    ...
    CREATE INDEX IF NOT EXISTS idx_dana_conversations_status ON dana_conversations(status);
    CREATE INDEX IF NOT EXISTS idx_dana_conversations_updated ON dana_conversations(updated_at DESC);
    """,
    "dana_messages": """
    CREATE TABLE IF NOT EXISTS dana_messages (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        conversation_id UUID NOT NULL REFERENCES dana_conversations(id) ON DELETE CASCADE,
        direction TEXT NOT NULL,
        text TEXT,
        llm_model TEXT,
        sent_at TIMESTAMPTZ DEFAULT NOW()
    );
    CREATE INDEX IF NOT EXISTS idx_dana_messages_conversation ON dana_messages(conversation_id, sent_at);
    """,
}
