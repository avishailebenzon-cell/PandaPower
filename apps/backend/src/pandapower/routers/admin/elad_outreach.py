"""Admin API for Elad Premium Client Outreach Campaigns (Session 35)"""
...
router = APIRouter(prefix="/admin/elad/outreach", tags=["admin", "elad", "outreach"])
...
async def create_campaign(
    ...
    try:
        ...
        auth = None  # TODO: Get from request context if needed
        ...
        # Create campaign
        result = await supabase.table("elad_outreach_campaigns").insert({
...
async def preview_campaign(
    ...
    try:
        ...
        # Get Elad's Green API config (for phone number)
        try:
            elad_client = await get_green_api_client("elad")
            elad_phone = elad_client.phone if hasattr(elad_client, 'phone') else "+972XXXXXXX"
        except Exception:
            elad_phone = "+972XXXXXXX"
...
async def send_campaign(
    ...
    try:
        ...
        # Get campaign
        campaign = await supabase.table("elad_outreach_campaigns").select("*").eq(
            "id", campaign_id
        ).single()
        ...
        # Get Elad Green API config
        elad_client = await get_green_api_client("elad")
        if not elad_client:
            raise HTTPException(status_code=500, detail="Elad WhatsApp not configured")
        ...
        for contact in contacts:
            ...
            rendered_msg = _substitute_placeholders(
                ...
                contact,
                company_name,
                elad_client.phone if hasattr(elad_client, 'phone') else "+972XXXXXXX"
            )
        ...
        if messages_to_create:
            await supabase.table("elad_outreach_messages").insert(messages_to_create).execute()
        ...
        # Queue Celery task
        try:
            from pandapower.workers.elad.outreach_sender import process_elad_outreach
            task = process_elad_outreach.delay(campaign_id)
...
async def get_campaign(
    ...
    try:
        result = await supabase.table("elad_outreach_campaigns").select("*").eq(
            "id", campaign_id
        ).single()
...
async def list_campaigns(
    ...
    try:
        result = await supabase.table("elad_outreach_campaigns").select("*").order(
